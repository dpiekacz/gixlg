...
<?php
   ...
   case 20:
    if ($ipnet == 1) {
     ...
     $res = mysql_query("SELECT * FROM `prefixes` WHERE (MBRCONTAINS(ip_poly, POINTFROMWKB(POINT($int_ip, 0))) && `length`!=0) ORDER BY LENGTH(aspath),`neighbor`,(neighbor+0),`neighbor`", $mid);
    }
    $nr = mysql_num_rows($res);
?>
<table id="rounded-corner">
